...
### Description  
This project demonstrates the design of several classes that form the basis for a dice-based game. The game consists of multiple classes: `Dice`, `Player`, `Board`, `Game`, and `Knucklebones`. These classes are designed using the principles of **composition** and **inheritance** to manage the gameplay, player statistics, and game mechanics.

### Files  

|   #   | File              | Description                                           |
| :---: | ----------------- | ----------------------------------------------------- |
|   1   | `dice.cpp`         | Contains the implementation of the `Dice` class.      |
|   2   | `player.cpp`       | Contains the implementation of the `Player` class.    |
|   3   | `board.cpp`        | Contains the implementation of the `Board` class.     |
|   4   | `game.cpp`         | Contains the implementation of the `Game` class.      |
|   5   | `knucklebones.cpp` | Contains the implementation of the `Knucklebones` class, which inherits from `Game`. |

### Prerequisites:  
- A working C++ compiler (e.g., GCC or Clang).  
- Ensure all source files are in the same directory for compilation.

### Instructions  

| Step | Action                           | Command/Format                                |
| ---- | -------------------------------- | --------------------------------------------- |
| 1    | **Write the Code**               | Implement the `Dice`, `Player`, `Board`, `Game`, and `Knucklebones` classes. |
| 2    | **Compile the Program**          | Compile using: `g++ *.cpp -o game_program`     |
| 3    | **Run the Program**              | Run the program with: `./game_program`         |
